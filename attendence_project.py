...
# Initializing Geofencing object
campus_center = (17.438188, 78.716631)  # Update with your actual coordinates
...
if n_people_in_pickle == len(people):
    ...
    while cap.isOpened():
        ...
        # Checking current location (by manually giving the current location for testing)
        ...
        if not geofence.is_within_geofence(current_location):
            cv2.putText(frame, "Outside Geofence.", (10, 50), cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 0, 255), 2)
            cv2.putText(frame, "Attendance Marking is NOT Allowed!", (10, 90), cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 0, 255), 2)
        ...
        for index, (loc, encode, landmark) in enumerate(zip(frame_face_loc, frame_face_encode, frame_face_landmarks)):
            ...
            if not frame_current_name == "Unknown":
                ...
                if utility.check_is_name_recorded(frame_current_name):
                    attendence_message = "Attendance Recorded!! Next Person"
                else:
                    attendence_message = ""
